...
class BackBone(torch.nn.Module):
    ...
    def __init__(self, input_dimensions, input_channels, output_channels, 
                 conv_count, batch_size, kernel_size=3):
...
class ReportFile:
    ...
    def set_dimensions(self):
        if self.dimensions[1] == 0:
            ...
            self.dimensions[1] = int_input("                  Input the number of convolutional layers per backbone: ")
    ...
    def set_params(self):
        print("         __________________________________________________________________")
        lr = float_input("                  Please input the desired learning rate (currently {}): ".format(
            self.learning_rate))

        wd = float_input("                  Please input the desired weight decay (currently {}): ".format(
            self.weight_decay))

        bs = int_input("                  Please input the desired batch size (currently {}): ".format(
            self.batch_size))
...
def main():
    ...
    report_path = "files/"+report.name + "/mainSIMPLE.report"
    csv_report_path = "files/"+report.name + "/" + report.name + ".csv"
    ...
    termination_accuracy = int_input("         Terminate after accuracy reaches: ")
    termination_epochs = int_input("         Terminate after epochs (overall) reach: ")
    ...
    while continuation:
        ...
        if autosave_type == 0:
            ...
            print("autosaves in a separate folder (recommended)")
        print("5: Continue without further adjustments")

        choice_3 = int_input("         Select the option: ")
        ...
        if choice_3 == 0:
            ...
            print("                  2: Autosave in a separate folder (recommended)")
            typed = int_input("                  Type the number of the selected option: ")
            if typed == 1 or typed == 2 or typed == 0:
                autosave_type = typed
            else:
                print("Invalid input")
        elif choice_3 == 5:
    ...
    while epoch <= termination_epochs and report.current_accuracy < termination_accuracy/100:
        ...
        if to_be_saved and autosave_type == 2:
